...
def construct_example_cfg(model_size, path=None, add_l0_module=False):
    ...
    if model_size == "9B":
        cfg = om.create({"name": "mosaic_llama_9b", "path": path,"init_device": "cpu", "d_model": 3584, 
                         "n_heads": 40, "n_layers": 28, "intermediate_size": 13824})
    
    # add default values
    cfg = om.merge(cfg, om.create({"max_seq_len": 4096, "vocab_size": 128256, "init_std": 0.02, "attn_pdrop": 0.0, "resid_pdrop": 0.0, "emb_pdrop": 0.0, "attn_impl": "flash", "rms_norm_eps": 1e-6}))
    if add_l0_module:
