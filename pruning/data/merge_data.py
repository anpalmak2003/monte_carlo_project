...
def merge_splits_with_no_bias():
    ...
    parser.add_argument("--output_dir", type=str, default='/home/jovyan/shares/SR006.nfs1/amaksimova/data/gigasets/gigasets_full_tokenized_mds/for_prune/')
    parser.add_argument("--input_dir", type=str, default='/home/jovyan/shares/SR006.nfs1/amaksimova/data/gigasets/gigasets_full_tokenized_mds/eval')
    parser.add_argument("--output_split", type=str, default='eval_merge')
    parser.add_argument("--split_names", nargs='+', default=["arxiv_sharded","frw_filtered_sharded","libgen_fiction_sharded","libgen_nougat_mathpix_sharded","libgen_sharded","pubmed_sharded","stackexchange_sharded","tribe_juridicial_sharded","tribe_medical_sharded","tribe_musdigest_sharded","wikiquote","cyberleninka_sharded","fandom_factology_no_urls","libgen_scihub_sharded","rucc_filtered_dclm_0.5","llemma-algebraic-stack-sharded","llemma-open-web-math-sharded","ru_cc_hf_sharded","starcoder_sharded","wikipedia_sharded"])
    ...
    if args.shuffle:
        for i, (input_dir, split_name) in enumerate(zip(args.input_dir, args.split_names)):
            ...
            data = load_data(str(input_dir), split_name, tokenizer_name = '/home/jovyan/shares/SR006.nfs1/amaksimova/models/9b/GigaChat-2-Lite-128k-preview')
